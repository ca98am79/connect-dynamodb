/*!
 * Connect - DynamoDB
 * Copyright(c) 2023 introvert.com LLC <support@introvert.com>
 * MIT Licensed
 */
/**
 * Module dependencies.
...
module.exports = function (connect) {
      ...
      return this.client
        .send(
          new GetItemCommand({
            TableName: this.table,
            Key: {
              [this.hashKey]: { [ScalarAttributeType.S]: sid },
            },
            ConsistentRead: true,
          })
        )
  ...
  DynamoDBStore.prototype.getParsedSession = function (result) {
    ...
    try {
      ...
      const sess = result.Item.sess[ScalarAttributeType.S];
  ...
  DynamoDBStore.prototype.set = function (sid, sess, fn) {
    this.initialize().then(() => {
      ...
      const params = {
        ...
        Item: {
          [this.hashKey]: {
            [ScalarAttributeType.S]: sid,
          },
          expires: {
            [ScalarAttributeType.N]: JSON.stringify(expires),
          },
          type: {
            [ScalarAttributeType.S]: "connect-session",
          },
          sess: {
            [ScalarAttributeType.S]: sessString,
          },
        },
      };
      ...
      const params = {
        ...
        ScanFilter: {
          expires: {
            AttributeValueList: [
              {
                [ScalarAttributeType.N]: now.toString(),
              },
            ],
  ...
  DynamoDBStore.prototype.touch = function (sid, sess, fn = () => {}) {
    this.initialize().then(() => {
      ...
      this.client
        ...
        .then((result) => {
          fn(null, {
            expires: result.Attributes.expires[ScalarAttributeType.N],
          });
        })
        .catch(fn);
    });
